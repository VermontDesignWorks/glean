...
def create_profile(user, memorg):
	profile = Profile(user=user, first_name="John", last_name="Doe", member_organization=memorg)
	profile.save()
	return profile
...
class GleanEventTests(TestCase):
	def test_happened(self):
		...
		upcomming_glean = create_glean(date = now + datetime.timedelta(days=5))
	...
	def test_render_directions(self):
		...
		self.assertEqual(glean.render_directions(), u'Follow the Map to the Right')
		...
		glean=create_glean(farm=farm,farm_location=farm_location,created_by=user)
		self.assertEqual(glean.render_directions(), u'Follow the Map to the Right')

		farm_location = create_farm_location(farm=farm, directions="Farm Location Directions")
		glean = create_glean(farm=farm, farm_location=farm_location, created_by=user)
		self.assertEqual(glean.render_directions(), farm_location.directions)

		farm_location = create_farm_location(farm=farm, directions="Farm Location Directions")
		glean = create_glean(farm=farm, farm_location=farm_location, created_by=user, directions="Glean Directions")
		self.assertEqual(glean.render_directions(), glean.directions)

		farm = create_farm(directions="Farm Directions")
		glean = create_glean(farm=farm, created_by=user, directions="Glean Directions")
		self.assertEqual(glean.render_directions(), glean.directions)
	...
	def test_render_instructions(self):
		user = create_volunteer_user_object()
		glean = create_glean(created_by=user)
		self.assertEqual(glean.render_instructions(), u"Show up early and have fun!")
		...
		farm_location = create_farm_location(farm=farm, instructions="Farm Location instructions")
		glean = create_glean(farm=farm, farm_location=farm_location, created_by=user)
		self.assertEqual(glean.render_instructions(), farm_location.instructions)

		farm_location = create_farm_location(farm=farm, instructions="Farm Location instructions")
		glean = create_glean(farm=farm, farm_location=farm_location, created_by=user, instructions="Glean instructions")
		self.assertEqual(glean.render_instructions(), glean.instructions)

		farm = create_farm(instructions="Farm instructions")
		glean = create_glean(farm=farm, created_by=user, instructions="Glean instructions")
		self.assertEqual(glean.render_instructions(), glean.instructions)

		glean = create_glean(created_by=user, instructions="Glean instructions")
		self.assertEqual(glean.render_instructions(), glean.instructions)
